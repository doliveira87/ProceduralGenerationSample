...
namespace Delaunay
{
	...
	public sealed class Halfedge: Delaunay.Utils.IDisposable
	{
		...
		public static Halfedge Create (Edge edge, Nullable<LR.Side> lr)
		{
			if (_pool.Count > 0) {
		...
		public Edge edge;
		public Nullable<LR.Side> leftRight;
		public Vertex vertex;
		
		// the vertex's y-coordinate in the transformed Voronoi space V*
		public float ystar;

		public Halfedge (Edge edge = null, Nullable<LR.Side> lr = null)
		{
			Init (edge, lr);
		}
		
		private Halfedge Init (Edge edge, Nullable<LR.Side> lr)
		{
			this.edge = edge;
			leftRight = lr;
		...
		internal bool IsLeftOf (Vector2 p)
		{
			...
			if (rightOfSite && this.leftRight == LR.Side.LEFT) {
				return true;
			}
			if (!rightOfSite && this.leftRight == LR.Side.RIGHT) {
				return false;
			}
			...
			return this.leftRight == LR.Side.LEFT ? above : !above;
		}
