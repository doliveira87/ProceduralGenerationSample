...
namespace Delaunay
{
	...
	sealed class EdgeReorderer: Utils.IDisposable
	{
		...
		private List<LR.Side> _edgeOrientations;
		public List<Edge> edges {
			get { return _edges;}
		}
		public List<LR.Side> edgeOrientations {
		...
		public EdgeReorderer (List<Edge> origEdges, VertexOrSite criterion)
		{
			...
			_edgeOrientations = new List<LR.Side> ();
		...
		private List<Edge> ReorderEdges (List<Edge> origEdges, VertexOrSite criterion)
		{
			...
			edge = origEdges [i];
			newEdges.Add (edge);
			_edgeOrientations.Add (LR.Side.LEFT);
			...
			while (nDone < n) {
				for (i = 1; i < n; ++i) {
					...
					if (leftPoint == lastPoint) {
						...
						_edgeOrientations.Insert (0, LR.Side.LEFT); // TODO: Change datastructure if this is slow
						newEdges.Insert (0, edge);
						done [i] = true;
					} else if (leftPoint == firstPoint) {
						firstPoint = rightPoint;
						_edgeOrientations.Insert (0, LR.Side.RIGHT);
						newEdges.Insert (0, edge);
						done [i] = true;
					} else if (rightPoint == lastPoint) {
						lastPoint = leftPoint;
						_edgeOrientations.Add (LR.Side.RIGHT);
						newEdges.Add (edge);
						done [i] = true;
					}
					if (done [i]) {
