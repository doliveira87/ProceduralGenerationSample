...
//			_plotBounds = null;
		...
		public List<Vector2> HullPointsInOrder ()
		{
			...
			List<LR.Side> orientations = reorderer.edgeOrientations;
			reorderer.Dispose ();
		...
		private void FortunesAlgorithm ()
		{
			...
			for (;;) {
				...
				if (newSite != null 
					...
					bisector = Halfedge.Create (edge, LR.Side.LEFT);
					...
					v = lbnd.vertex;
					v.SetIndex ();
					lbnd.edge.SetVertex ((LR.Side)lbnd.leftRight, v);
					rbnd.edge.SetVertex ((LR.Side)rbnd.leftRight, v);
					edgeList.Remove (lbnd); 
					heap.Remove (rbnd);
					edgeList.Remove (rbnd); 
					leftRight = LR.Side.LEFT;
		...
		private Site FortunesAlgorithm_rightRegion (Halfedge he)
		{
			...
			return edge.Site (SideHelper.Other ((LR.Side)he.leftRight));
		}
